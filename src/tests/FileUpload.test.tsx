...
describe('FileUpload Component', () => {
  ...
  it('displays remaining slots', () => {
    const uploadedFiles = Array.from({ length: 7 }, (_, i) =>
      ({ file: new File(['content'], `test${i}.pdf`, { type: 'application/pdf' }), id: `id-${i}`, previewUrl: '' })
    );
  ...
  it('validates file types', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Invalid file type. Only PDFs are allowed. Skipped: test.txt')).toBeInTheDocument();
    });
  });

  it('validates file size', async () => {
    const largeFile = { file: new File([new ArrayBuffer(201 * 1024 * 1024)], 'large.pdf', { type: 'application/pdf' }), id: 'id-1', previewUrl: '' };
    ...
    await waitFor(() => {
      expect(screen.getByText(/Large Total Size/)).toBeInTheDocument();
    });
  });

  it('prevents duplicate files', async () => {
    const existingFile = { file: new File(['content'], 'test.pdf', { type: 'application/pdf' }), id: 'id-1', previewUrl: '' };
  ...
  it('allows removing files', () => {
    const files = [{ file: new File(['content'], 'test.pdf', { type: 'application/pdf' }), id: 'id-1', previewUrl: '' }];
